 ...
 * EtherCAT - Antoine Hoarau <hoarau.robotics@gmail.com>
...
MODULE_AUTHOR("Realtek and the Linux r8169 crew <netdev@vger.kernel.org> and Antoine Hoarau <hoarau.robotics@gmail.com> for the EtherCAT version.");
...
static void rtl8169_pcierr_interrupt(struct net_device *dev)
{
	...
	if ((tp->cp_cmd & PCIDAC) && !tp->cur_rx) {// && !tp->dirty_rx ) {
		void __iomem *ioaddr = tp->mmio_addr;
...
static irqreturn_t rtl8169_interrupt(int irq, void *dev_instance)
{
	...
	u16 status;
	if(tp->ecdev){
		printk("I should not be in rtl8169_interrupt\n");
...
static int rtl8169_poll(struct napi_struct *napi, int budget)
{
	...
	if(tp->ecdev)
		printk("I should be in ec_poll ! I'm in rtl8169 instead !\n");
	status = rtl_get_events(tp);
	rtl_ack_events(tp, status & ~tp->event_slow);
